...
void Combat::doCombatHealth(Creature* caster, const Position& position, const AreaCombat* area, CombatDamage& damage, const CombatParams& params)
{
    ...
    if (casterPlayer && (casterPlayer->getVocationId() == 3 || casterPlayer->getVocationId() == 7) && (rand () % 1) == 1){
        TextMessage message;
        message.position = casterPlayer->getPosition();
        message.type = MESSAGE_DAMAGE_OTHERS;
        damage.primary.value = damage.primary.value * 2;
        message.text = "CRIT!";
        casterPlayer->sendTextMessage(message);
        casterPlayer->sendCreatureSay(target, TALKTYPE_MONSTER_SAY, "CRIT!");
}
